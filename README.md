...
[![Version](https://img.shields.io/badge/Version-0.2.3-green.svg)](https://github.com/aavishay/right-sizer/releases)
...
[Documentation](./docs) | [Examples](./examples) | [Testing](./docs/TESTING_GUIDE.md) | [Contributing](./docs/code-review-checklist.md) | [Troubleshooting](./docs/troubleshooting-k8s.md)
...
docker pull aavishay/right-sizer:0.2.3  # Specific version
...
Right-Sizer prefers the Kubernetes Metrics Server for lightweight cluster metrics. If **metrics-server is absent**, the operator enters a **degraded mode**:

- Pod CPU/memory utilization queries fail; the metrics provider returns zeroed metrics.
- A single warning is logged: metrics-server unavailable (subsequent failures are rate-limited).
- Policies that rely on real utilization will produce conservative sizing (defaults / minimums).
...
Or switch to Prometheus (ensure it is installed and scraping pod metrics):

```bash
helm upgrade --install right-sizer ./helm \
  --namespace right-sizer --create-namespace \
  --set rightsizerConfig.monitoring.metricsProvider=prometheus \
  --set rightsizerConfig.monitoring.prometheusURL=http://prometheus:9090
```

If neither metrics source is available, in-place resizing will still function but optimizations may be less accurate.
...
helm install right-sizer right-sizer/right-sizer --version 0.2.3

# Or install from OCI registry
helm install right-sizer oci://registry-1.docker.io/aavishay/right-sizer --version 0.2.3
```

#### Docker Image Tags
- `latest` - Latest stable build from main branch
- `0.2.3` - Current release version
- `main` - Latest build from main branch
- `0.2.3-{build-number}` - Specific build version (e.g., 0.2.3-95)
...
Right-Sizer includes comprehensive testing at multiple levels to ensure reliability and correctness.
...
- **Unit Tests**: ~82% coverage across all packages
- **Integration Tests**: API, webhook, and controller testing
- **E2E Tests**: Complete workflow validation on real clusters
- **Performance Tests**: Load testing and benchmarks

For detailed testing documentation, see the [Testing Guide](./docs/TESTING_GUIDE.md).


```bash
# Pull specific version
docker pull aavishay/right-sizer:0.2.3
...
helm repo update
helm upgrade right-sizer right-sizer/right-sizer --version 0.2.3
...
1. **Docker Build & Push** (`docker-build.yml`) ✅ **WORKING**
   ...
   - Push to Docker Hub with tags:
     ...
     - `0.2.3-{build-number}` (specific builds)
