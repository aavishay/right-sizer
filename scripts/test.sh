...
Run tests for the right-sizer project.
...
OPTIONS:
  ...
  -s    Run only short tests
  -t    Set test timeout (default: 10m)
  -p    Set parallel test execution (default: 4)
...
# Capture go vet output but don't fail the script
...
if [ "$SHORT" != true ]; then
  print_info "Running race detector on short tests..."
  go test -race -short ./...
